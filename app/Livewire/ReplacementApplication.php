...
class ReplacementApplication extends Component
{
    ...
    public function register2()
    {
        ...
        $existingApplication = Applications::where('user_id', auth()->id())
            ->where('application_type', 'Replacement Application')
            ->first();

        if ($existingApplication) {
            // Redirect the user back with an error message
            return redirect()->back()->with('error', 'You have already applied for an ID replacement. You cannot apply for a second one.');
        }
